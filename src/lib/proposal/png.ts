...
import { buildProposalHtml, WIDTH, HEADER_HEIGHT, CARD_HEIGHT, ITEMS_PER_PAGE, type PriceDisplay } from "./template";
...
async function renderOnePng(items: ProposalItem[], priceDisplay: PriceDisplay): Promise<Buffer> {
  ...
  const height = HEADER_HEIGHT + items.length * CARD_HEIGHT;
...
export async function renderProposalPng(
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    zip.file(`netdoi-proposal-${i + 1}.png`, png);
  }
